...
export class ListReclamationComponent implements OnInit {
  ...
  deleteItem() {
    ...
    this.isLoading = true
    this.reclamationServ.delete(this.idDeleteModal)
      .subscribe(
        res => {
  ...
  openModalDelete(id, params2) {
    ...
    this.params1Delete = "La reclamation"
  ...
  getReclamations() {
    ...
    this.isLoading = true
    this.reclamationServ.getAll(this.request)
    .subscribe(
      res => {
        this.isLoading = false
        let resultat: any = res
        if (resultat.status) {
          this.reclamations = resultat.resultat.docs
  ...
  titreFile = "Liste de reclamations"
  nameFile = "liste_reclamations"
  printout() {
    this.utilite.printout(this.reclamations, this.items, this.titreFile)
  }

  generatePDF() {
    this.utilite.generatePDF(this.reclamations, this.items, this.titreFile, this.nameFile)
  }

  exportexcel() {
    /* table id is passed over here */
    this.utilite.exportexcel(this.reclamations, this.items, this.titreFile, this.nameFile)
  }
  ...
  ouvrirFenetreAjout(){
    ...
    this.router.navigateByUrl('/reclamation/ajout');

  }
  ouvrirFenetreModifier(id){
 
   this.currentId=id;
   this.reclamationServ.currentID=this.currentId;

  this.Tr = this.reclamationServ.get(id).subscribe(
  res => {
    let resultat: any = res
    if (resultat.status) {
