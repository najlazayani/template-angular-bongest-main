...
@Component({
  selector: 'app-list-typeequipement',
  templateUrl: './list-typeequipement.component.html',
  styleUrls: ['./list-typeequipement.component.scss']
})
export class ListTypeequipementComponent implements OnInit, OnDestroy {
  ...
  currentId ="";
   currentTr ="" ;
   Tr ;
   typeEquipemnts : TypeEquipement[]=[];
   private typeEquipementSubscription :Subscription;
  ...
  deleteItem() {
    ...
    this.isLoading = true
    this.typeEquipementService.delete(this.idDeleteModal)
      .subscribe(
        res => {
  ...
  openModalDelete(id, params2) {
    ...
    this.params1Delete = "Le type d'equipement"
  ...
  ngOnDestroy() {
    this.typeEquipementSubscription.unsubscribe();
  }
  ...
  getTypeEquipements() {
    ...
    this.isLoading = true
    this.typeEquipementService.getAll(this.request)
    .subscribe(
      res => {
        this.isLoading = false
        let resultat: any = res
        if (resultat.status) {
          this.typeEquipements = resultat.resultat.docs
  ...
  titreFile = "Liste de TypeEquipements"
  nameFile = "liste_TypeEquipements"
  printout() {
    this.utilite.printout(this.typeEquipements, this.items, this.titreFile)
  }

  generatePDF() {
    this.utilite.generatePDF(this.typeEquipements, this.items, this.titreFile, this.nameFile)
  }

  exportexcel() {
    /* table id is passed over here */
    this.utilite.exportexcel(this.typeEquipements, this.items, this.titreFile, this.nameFile)
  }
